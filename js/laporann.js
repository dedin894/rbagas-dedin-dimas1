...
function setupModals() {
    ...
    if (addLaporanBtn && addLaporanModal) {
        addLaporanBtn.addEventListener('click', function() {
            addLaporanModal.style.display = 'block';
        });
    }
    // Tutup modal
...
function setupLoginForm() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            ...
            if (email === 'apod@gmail.com' && password === '123') {
            ...
            if (pengiriman) {
                ...
                resultDiv.innerHTML = `
                    <div class="tracking-details">
                        ...
                        <p><strong>Nama Pembeli:</strong> ${pengiriman.namaMahasiswa}</p>
...
// Data awal laporan penjualan obat
let dataLaporan = [
    { no: 1, nama: "Paracetamol", jenis: "Tablet", terjual: 50, harga: 2500 },
    { no: 2, nama: "Amoxicillin", jenis: "Kapsul", terjual: 30, harga: 3500 }
];

// Fungsi hitung total penjualan
function totalPenjualan(item) {
    return item.terjual * item.harga;
}

// Tampilkan data laporan ke tabel
function displayLaporan() {
    ...
    dataLaporan.forEach((item, index) => {
        ...
        row.innerHTML = `
            ...
            <td>Rp ${totalPenjualan(item).toLocaleString()}</td>
        `;
...
// Tambah laporan baru
function setupAddLaporan() {
    const form = document.getElementById("addStockForm");

    form.addEventListener("submit", (e) => {
        e.preventDefault();

        const no = document.getElementById("kodeBahan").value;
        const nama = document.getElementById("namaBahan").value;
        const jenis = document.getElementById("stok").value;
        const terjual = parseInt(document.getElementById("status").value);
        const harga = parseInt(document.getElementById("hargaSatuan").value);

        dataLaporan.push({ no, nama, jenis, terjual, harga });
